...
import com.project.module.bo.common.auth.filter.CustomAuthenticationFilter;
import com.project.module.bo.common.auth.filter.JwtAuthorizationFilter;
import com.project.module.bo.common.auth.handler.CustomAuthFailureHandler;
import com.project.module.bo.common.auth.handler.CustomAuthSuccessHandler;
import com.project.module.bo.common.auth.handler.CustomAuthenticationProvider;
import com.project.module.bo.common.auth.handler.CustomLogoutHandler;
...
/**
 * Spring Security 환경 설정을 구성하기 위한 설정 클래스
 * 웹 서비스가 로드 될때 Spring Container 의해 관리가 되는 클래스이며 사용자에 대한 ‘인증’과 ‘인가’에 대한 구성을 Bean 메서드로 주입한다
 */
@Slf4j
@Configuration
@EnableWebSecurity
public class SecurityConfig {
        ...
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/swagger-ui/**",
                                "/api/v1/user/join",
                                "/v3/api-docs/**",
                                "/api/test/token").permitAll()               // Swagger UI 접근 허용
    ...
    /**
     * 4. '인증' 제공자로 사용자의 이름과 비밀번호를 데이터베이스에 제공하여 반환
     * - 과정: CustomAuthenticationFilter → AuthenticationManager(interface) → CustomAuthenticationProvider(implements)
     *
    ...
    /**
     * 12. 로그아웃 처리를 위한 Handler를 커스텀으로 구성한다.
     *
     * @return
     */
    @Bean
    public LogoutHandler securityCustomLogoutHandler() {
